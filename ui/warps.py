...
#from ui import gridpanels
#import ui.gridpanels as gridpanels

class CursorWarp(Warp):
    def onTransferFrom(self, other):
        self.window.gridPanel.children = [self.cursorFlyer]
    def onTransferTo(self, other):
    ...
    def warpCancelKey(self):
        self.transfer(RequestActionWarp(self.interf))

"""
class ExamineWarp(CursorWarp):
    def __init__(self, interf):
        super().__init__(interf)
        self.cursorFlyer = gridpanels.Cursor(self.window.gridPanel, self.state.hero.xyPos )
"""
